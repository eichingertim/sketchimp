...
UPLOAD_PATH = path.resolve(__dirname, "..", Constants.AVATAR_STORAGE),
AvatarStorage = function(options) {

    function AvatarStorage(opts) {
        var baseUrl = Constants.AVATAR_BASE_URL,
...
class StorageWorker {
    constructor() {
        this.AvatarStorage = AvatarStorage(Constants.UPLOAD_OPTIONS);
        this.fileFilter = fileFilter;
        this.upload = multer({
            storage: this.AvatarStorage,
            limits: {
                files: 1,
                fileSize: Constants.UPLOAD_MAX_SIZE,
            },
