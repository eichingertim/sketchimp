    ...
    router.post("/join/:id", (req, res)=>{
        try {
            ...
            joinPromise.then((channelProfile) => {
                dbWrapper.User.addChannelToList(req.user, channelProfile.id);
                return res.status(Constants.HTTP_STATUS_CODES.OK).json(new ApiResponse(1, Constants.RESPONSEMESSAGES.CHANNEL_JOIN_SUCCESS, channelProfile));
            }).catch((e)=>{
                console.log(e);
                return res.status(Constants.HTTP_STATUS_CODES.OK).json(new ApiResponse(0, e));
            });
            ...
            if (channel) {
                if (req.user) {
                    ...
                    return res.status(Constants.HTTP_STATUS_CODES.OK)
                        .json(new ApiResponse((success) ? 1 : 0, (success) ? Constants.RESPONSEMESSAGES.CHANNEL_LEAVE_SUCCESS : Constants.RESPONSEMESSAGES.CHANNEL_LEAVE_FAIL, 
    ...
    router.post("/delete/:id", middleware.userIsChannelCreator, (req, res)=>{
        ...
        try {
            ...
            deletePromise.then((memberList) => {
                ...
                return res.status(Constants.HTTP_STATUS_CODES.OK).json(new ApiResponse(1, Constants.RESPONSEMESSAGES.DELETE_CHANNEL_SUCCESS, memberList));
            }).catch((e)=>{
                console.log(e);
                return res.status(Constants.HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json(new ApiResponse(0, e));
            });
