...
function setupRoutes(channelRoute, router) {
    ...
    router.post("/delete/:id", middleware.userIsChannelCreator, (req, res)=>{
        ...
        if (!deletePromise) {
            return res.status(Constants.HTTP_STATUS_CODES.OK).json(new ApiResponse(Constants.RESPONSEMESSAGES.DELETE_CHANNEL_FAIL));
        }
        deletePromise.then((memberList) => {
            console.log(memberList);
            dbWrapper.User.removeChannelFromList(req.user, req.params.id);
            memberList.forEach((memberId) => {
                console.log("removing channel from" + memberId);
                dbWrapper.User.removeChannelFromList(memberId, req.params.id);
            });
            ...
            console.log(e);
            return res.status(Constants.HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).json(new ApiResponse(Constants.RESPONSEMESSAGES.DELETE_CHANNEL_FAIL));
        });
    });
