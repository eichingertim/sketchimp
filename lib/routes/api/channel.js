...
function setupRoutes(channelRoute, router) {
    ...
    router.post("/role/:id/:userId/:role", middleware.userIsChannelAdmin, (req, res) => {
        console.log(req.params);
        try {
            let rolePromise = dbWrapper.Channel.changeRoleForUserId(req.params.id, req.params.userId, req.params.role);
            rolePromise.then((channelProfile) => {
                return res.status(Constants.HTTP_STATUS_CODES.OK).json(new ApiResponse(channelProfile));
            })
            .catch((e) => {
                console.log(e);
                return res.status(Constants.HTTP_STATUS_CODES.OK).json(new ApiResponse(e));
