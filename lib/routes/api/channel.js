...
function setupRoutes(channelRoute, router) {
    ...
    router.get("/:id", middleware.userIsChannelParticipant, (req, res)=>{
        ...
        if (channel) {
            ...
            return res.status(Constants.HTTP_STATUS_CODES.OK).json(new ApiResponse(Constants.RESPONSEMESSAGES.CHANNEL_FOUND, dbWrapper.Channel.formChannelProfile(channel)));
        }
    });
            ...
            if (channel) {
                if (req.user) {
                    ...
                    return res.status(Constants.HTTP_STATUS_CODES.OK)
                        .json(new ApiResponse((success) ? Constants.RESPONSEMESSAGES.CHANNEL_LEAVE_SUCCESS : Constants.RESPONSEMESSAGES.CHANNEL_LEAVE_FAIL, 
                            dbWrapper.Channel.formChannelProfile(channel)));
                }
            }
        ...
        try {
            if (channel) {
                if (req.user) {
                    ...
                    return res.status(Constants.HTTP_STATUS_CODES.OK)
                        .json(new ApiResponse((success) ? Constants.RESPONSEMESSAGES.CHANNEL_KICK_SUCCESS : Constants.RESPONSEMESSAGES.CHANNEL_KICK_FAIL, 
                            dbWrapper.Channel.formChannelProfile(channel)));
                }
            }
            return res.sendStatus(Constants.HTTP_STATUS_CODES.BAD_REQUEST);
    ...
    router.post("/roles/:id", middleware.userIsChannelAdmin, (req, res) => {
        ...
        try {
            ...
            return res.status(Constants.HTTP_STATUS_CODES.BAD_REQUEST).json(new ApiResponse(e));
        }
    });
}
