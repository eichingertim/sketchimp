...
function setupRoutes(sketchRoute, router) {
    ...
    router.post("/new/:channelId", middleware.userIsLoggedIn, (req, res) => {
        ...
        sketchPromise = dbWrapper.Sketch.createNewForChannelId(req.params.channelId, req.body.name);
        if(!sketchPromise){
    ...
    router.post("/upvote/:sketchId", middleware.userIsLoggedIn, (req, res) => {
        let sketchPromise = dbWrapper.Sketch.addUserToUpvotes(req.params.sketchId, req.params.user._id);
    ...
    router.post("/downvote/:sketchId", middleware.userIsLoggedIn, (req, res) => {
        let sketchPromise = dbWrapper.Sketch.addUserToDownvotes(req.params.sketchId, req.params.user._id);
    ...
    router.post("/finalize-create/:channelId", middleware.userIsLoggedIn, StorageWorker.uploadSketch.any(),(req, res) => {
        let channelPromise = dbWrapper.Channel.getForId(req.params.channelId),
        sketchPromise,
        files,
        ...
        sketchPromise = dbWrapper.Sketch.finalizeCurrentAndCreateNew(req.params.channelId, files[0], req.body.name);
        if (!sketchPromise) {
            return res.status(Constants.HTTP_STATUS_CODES.OK).json(new ApiResponse(Constants.RESPONSEMESSAGES.SKETCH_FINALIZE_SUCCESS));
        }
