...
function setupRoutes(sketchRoute, router) {
    ...
    router.post("/new/:channelId", middleware.userIsLoggedIn, (req, res) => {
        ...
        sketchPromise = dbWrapper.Sketch.createNewForChannelId(req.params.channelId, req.params.sketchName);
        if(!sketchPromise){
    ...
    router.post("/save/:channelId", middleware.userIsLoggedIn, (req, res) => {
        ...
        sketchPromise = dbWrapper.Sketch.updateCurrentForChannelId(req.params.channelId, req.params.data);
        if(!sketchPromise){
            console.log(Constants.RESPONSEMESSAGES.SKETCH_NOT_CREATED);
