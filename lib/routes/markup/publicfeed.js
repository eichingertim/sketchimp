  ...
  try {
    ...
    sketchPromise.then((published) => {
      if(req.user){
        ...
        if(!userPromise){
          return res.status(Constants.HTTP_STATUS_CODES.OK).render("publicfeed", {user: undefined, sketches: published, title: "publicfeed"});
        }

        userPromise.then((rUser) => {
          return res.status(Constants.HTTP_STATUS_CODES.OK).render("publicfeed", {user: rUser, sketches: published, title: "publicfeed"});
        });
      }else{
        return res.status(Constants.HTTP_STATUS_CODES.OK).render("publicfeed", {user: undefined, sketches: published, title: "publicfeed"});
      }    
