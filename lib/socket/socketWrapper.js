    ...
    io.on("connection", (client) => {
        ...
        client.on("subscribe", function (channelId) {
            ...
            if (appServer.channels[channelId] !== undefined && appServer.channels[channelId].users !== undefined) {
                ...
                client.emit("template", {channelId: channelId, templateUrl: appServer.channels[channelId].templateUrl });
            }
        });
        ...
        client.on("admin-settings", (data) => {
            io.in(data.channelId).emit("admin-settings", data);
        });

        client.on("template", (data) => {
            appServer.channels[data.channelId].templateUrl = data.templateUrl;
            io.in(data.channelId).emit("template", data);
        });

        client.on("new-sketch", (data) => {
            io.in(data.channelId).emit("new-sketch", data);
        });

        client.on("delete-channel", (data) => {
            appServer.channels[data.channelId] = null;
            io.in(data.channelId).emit("channel-delete", null);
        });
    });
}
