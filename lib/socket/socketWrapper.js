...
function setupSocketIO(appServer, io) {
    io.on("connection", (client) => {
        ...
        client.on("subscribe", function(channelId) {
            ...
            if (appServer.channels[channelId] !== undefined && appServer.channels[channelId].users !== undefined) {
                ...
                for (let i in appServer.channels[channelId].users) {
                    if (appServer.channels[channelId].users[i].lineHistory === undefined) continue;
        ...
        client.on("clear-canvas", (data) => {
            if (data.sketchData !== null) {
                appServer.channels[data.channelId].users = {};
                io.in(data.channelId).emit("clear-canvas", {sketchData: data.sketchData, userRole: null, isMultiLayer: null});
            } else if (data.userRole !== null){
                if (data.userRole === Constants.ROLES.ADMIN) {
                    appServer.channels[data.channelId].users = {};
                } else {
                ...
                io.in(data.channelId).emit("clear-canvas", {sketchData: null, userRole: data.userRole, isMultiLayer: data.isMultiLayer});
            }
        });
    });
}
