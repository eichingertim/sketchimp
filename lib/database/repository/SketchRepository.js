...
class SketchRepository extends Observable {
    ...
    createNewForChannelId(channelId, SketchName){
        ...
        let finalizePromise = this.finalizeCurrentForChannelId(channelId);
    ...
    finalizeCurrentAndCreateNew(channelId, filePath, newSketchName){
        if(!ObjectID.isValid(channelId) || !filePath){
            return;
        }
        return Sketch.getCurrentForChannelId(channelId).then((rSketch) => {
            rSketch.filePath = filePath;
            rSketch.finalized = true;
            rSketch.data = "";
            rSketch.save();
            return this.createNewForChannelId(channelId, newSketchName);
        }).catch((e) => {
            console.log(e);
            return;
        });
