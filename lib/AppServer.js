...
class AppServer {
  ...
  start(port) {
    ...
    io.on('connection', (socket) => {
      ...
      for (var i in this.line_history) {
        socket.emit('line', {
          line: this.line_history[i].line,
          color: this.line_history[i].color,
          penRubber: this.line_history[i].penRubber
        });
      }

      socket.on('line', (data) => {
        this.line_history.push(data);
        io.emit('line', {line: data.line, color: data.color, penRubber: data.penRubber});
      })
    });
  }
